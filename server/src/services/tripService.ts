  ...
  const fn = db.transaction(() => {
    ...
    const insertPlace = db.prepare(`
      INSERT INTO places (trip_id, name, description, lat, lng, address, category_id, price, currency,
        ...
        duration_minutes, notes, image_url, google_place_id, google_ftid, google_maps_url, website, phone, transport_mode, osm_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    for (const p of oldPlaces) {
      const r = insertPlace.run(newTripId, p.name, p.description, p.lat, p.lng, p.address, p.category_id,
        ...
        p.google_ftid, p.google_maps_url, p.website, p.phone, p.transport_mode, p.osm_id);
