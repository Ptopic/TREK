...
type PlaceLike = Pick<Place | AssignmentPlace, 'name' | 'lat' | 'lng' | 'google_place_id' | 'google_ftid' | 'google_maps_url'>
...
export function getGoogleMapsUrlForPlace(place: PlaceLike | null | undefined, detailsUrl?: string | null): string | null {
  ...
  const storedGoogleMapsUrl = place.google_maps_url?.trim()
  if (storedGoogleMapsUrl && isGoogleMapsUrl(storedGoogleMapsUrl)) return storedGoogleMapsUrl
